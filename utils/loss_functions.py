...
if __name__ == "__main__":
    ...
    img1 = tf.random.uniform((1, 256, 256, 1), dtype=tf.float32)
    img2 = tf.random.uniform((1, 256, 256, 1), dtype=tf.float32)

    ssim_loss = SSIM_LOSS(img1, img2)
    l1_loss = L1_LOSS(img1)
    fro_loss = Fro_LOSS(img1)

    print(f"SSIM Loss: {ssim_loss.numpy()}")
    print(f"L1 Loss: {l1_loss.numpy()}")
    print(f"Frobenius Loss: {fro_loss.numpy()}")
